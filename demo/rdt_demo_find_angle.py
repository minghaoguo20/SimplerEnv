...
def _main(angle, args):
    ...
    obs_notebook = {}
    print(f"init pose: {actors.tcp.pose.p} \t q: {actors.tcp.pose.q} \t angle: {coordination_transform.quaternion_to_axis_angle(actors.tcp.pose.q)}")
    ...
    while not (done or truncated) and len(stage) > 0:
        image_info = ""
        image_info += f"angle:{visualizer.nparray_to_string(angle, DIGITS)}\n"
        ...
        # action = key_points_op.cal_action(actors, object_point, grasp_state, current_stage, stage, key_points)
        # #####################################################################
        ...
        if gripper_2d is not None:
            ...
            _gripper_angle = visualizer.nparray_to_string(coordination_transform.quaternion_to_axis_angle(actors.tcp.pose.q), DIGITS)
            ...
            draw.text(
                (gripper_2d[0] + draw_w_offset.gripper[0], gripper_2d[1] + draw_w_offset.gripper[1] + 3 * FONT_SIZE),
                f"Angle: {_gripper_angle}",
                fill="green",
                font=font,
            )
        ...
        image_info += f"First: {visualizer.nparray_to_string(key_points.first.p3d.p, DIGITS)} -> {visualizer.nparray_to_string(key_points.post.p3d.p, DIGITS)}\n"
